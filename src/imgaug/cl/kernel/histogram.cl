...
void partial_histogram_pln(__global unsigned char *input,
    ...
    int localSize = (int)get_local_size(0) * (int)get_local_size(1);
    int groupIndex = (get_group_id(1) * get_num_groups(0) + get_group_id(0)) * 256 * channel;

    local unsigned char tempHistogram [ 256 * channel];
                      ...
                      const unsigned int channel)
{
    ...
    int
    n = num_groups;
    local uint tmp_histogram[256 * channel];
