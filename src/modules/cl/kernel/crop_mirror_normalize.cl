...
kernel void crop_mirror_normalize_batch(
    __global unsigned char
        *input, // Input Tensor NHCW or NCHW (Depending on Planar or Packed)
    __global unsigned char
        *output, // Output Tensor (For now of type RPP8U), FLOAT32 will be given
  ...
  unsigned long src_pixIdx =
      ...
      (id_x + start_x[id_z] + (id_y + start_y[id_z]) * max_src_width[id_z]) *
          plnpkdindex; //((id_x + start_x[id_z])  + (id_y + start_y[id_z]) *
                       //max_src_width[id_z] ) * plnpkdindex ;
  if (local_flip == 1) {
  ...
  unsigned long dst_pixIdx =
      ...
      (id_x + id_y * max_dst_width[id_z]) *
          plnpkdindex; // output pix id will change according to format as well
                       // //TODO
  if ((id_x < dst_width[id_z]) && (id_y < dst_height[id_z])) {
    for (indextmp = 0; indextmp < channel; indextmp++) {
      output[dst_pixIdx] = input[src_pixIdx]; //(unsigned char)saturate_8u(255.0
                                              //* ((input[src_pixIdx] -
                                              //local_mean) / local_std_dev));
...
kernel void crop_mirror_normalize_batch_fp16(
    __global half
        *input, // Input Tensor NHCW or NCHW (Depending on Planar or Packed)
    __global half *output, // Output Tensor (For now of type RPP8U), FLOAT32
                           // will be given depending on necessity
    __global unsigned int *dst_height, __global unsigned int *dst_width,
    __global unsigned int *src_width, __global unsigned int *start_x,
    __global unsigned int *start_y, __global float *mean,
    __global float *std_dev, __global unsigned int *flip,
...
kernel void crop_mirror_normalize_batch_fp32(
    __global float
        *input, // Input Tensor NHCW or NCHW (Depending on Planar or Packed)
    __global float *output, // Output Tensor (For now of type RPP8U), FLOAT32
                            // will be given depending on necessity
    __global unsigned int *dst_height, __global unsigned int *dst_width,
    __global unsigned int *src_width, __global unsigned int *start_x,
    __global unsigned int *start_y, __global float *mean,
