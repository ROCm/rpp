                                    ...
                                    )
{
    ...
    if((id_y >= yroi_begin[id_z] ) && (id_y <= yroi_end[id_z]) && (id_x >= xroi_begin[id_z]) && (id_x <= xroi_end[id_z]))
    {
        ...
        pixel = convert_one_pixel_to_rgb(hsv); // Converting to RGB back with hue modification
        output[pixIdx] =        = alpha1 * pixel.x + beta1;
        output[pixIdx + l_inc]  = alpha1 * pixel.y + beta1;
        output[pixIdx + 2*l_inc] = alpha1 * pixel.z + beta1;
    }
     else {
        output[pixIdx] = pixel.x;
        output[pixIdx + l_inc] =  pixel.y;
                                    ...
                                    )
{
    ...
    if((id_y >= yroi_begin[id_z] ) && (id_y <= yroi_end[id_z]) && (id_x >= xroi_begin[id_z]) && (id_x <= xroi_end[id_z]))
    {
        ...
        pixel = convert_one_pixel_to_rgb(hsv); // Converting to RGB back with hue modification
        output[pixIdx] = (half)(alpha1 * pixel.x + beta1);
        output[pixIdx + l_inc] = (half)(alpha1 * pixel.y + beta1);
        output[pixIdx + 2*l_inc] = (half)(alpha1 * pixel.z + beta1);
    }
     else {
        output[pixIdx] = pixel.x;
        output[pixIdx + l_inc] =  pixel.y;
