...
#pragma omp parallel for num_threads(dstDescPtr->n)
    for(int batchCount = 0; batchCount < dstDescPtr->n; batchCount++)
    {
        ...
        auto load48FnPkdPln = &rpp_load48_u8pkd3_to_f32pln3_mirror_avx;
        auto load48FnPlnPln = &rpp_load48_u8pln3_to_f32pln3_mirror_avx;
        auto load16Fn = &rpp_load16_u8_to_f32_mirror_avx;
        ...
        else if(flipAxis == 1)
        {
            ...
            rowPtrMirror = 1;
            load48FnPkdPln = &rpp_load48_u8pkd3_to_f32pln3_avx;
            load48FnPlnPln = &rpp_load48_u8pln3_to_f32pln3_avx;
            load16Fn = &rpp_load16_u8_to_f32_avx;
        }
        else
        {
    ...
    for(int batchCount = 0; batchCount < dstDescPtr->n; batchCount++)
    {
        ...
        if ((srcDescPtr->c == 3) && (srcDescPtr->layout == RpptLayout::NHWC) && (dstDescPtr->layout == RpptLayout::NCHW))
        {
            ...
            for(int i = 0; i < roi.xywhROI.roiHeight; i++)
            {
                ...
                for (; vectorLoopCount < bufferLength; vectorLoopCount += 3)
                {
                    *dstPtrTempR = (Rpp8s) RPPPIXELCHECKI8((Rpp32f) (srcPtrTemp[0] + 128) - 128);
                    *dstPtrTempG = (Rpp8s) RPPPIXELCHECKI8((Rpp32f) (srcPtrTemp[1] + 128) - 128);
                    *dstPtrTempB = (Rpp8s) RPPPIXELCHECKI8((Rpp32f) (srcPtrTemp[2] + 128) - 128);
        ...
        else if ((srcDescPtr->c == 3) && (srcDescPtr->layout == RpptLayout::NCHW) && (dstDescPtr->layout == RpptLayout::NHWC))
        {
            ...
            for(int i = 0; i < roi.xywhROI.roiHeight; i++)
            {
                ...
                for (; vectorLoopCount < bufferLength; vectorLoopCount++)
                {
                    dstPtrTemp[0] = (Rpp8s) RPPPIXELCHECKI8((Rpp32f) ((*srcPtrTempR) + 128) - 128);
                    dstPtrTemp[1] = (Rpp8s) RPPPIXELCHECKI8((Rpp32f) ((*srcPtrTempG) + 128) - 128);
                    dstPtrTemp[2] = (Rpp8s) RPPPIXELCHECKI8((Rpp32f) ((*srcPtrTempB) + 128) - 128);
        ...
        else
        {
            ...
            for(int c = 0; c < layoutParams.channelParam; c++)
            {
                ...
                for(int i = 0; i < roi.xywhROI.roiHeight; i++)
                {
                    ...
                    for (; vectorLoopCount < bufferLength; vectorLoopCount++)
                    {
                        *dstPtrTemp = (Rpp8s) RPPPIXELCHECK((Rpp32f) ((*srcPtrTemp) + 128) - 128);
