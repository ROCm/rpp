                                 ...
                                 rpp::Handle& handle)
{
    ...
    float rainPercent = rainPercentage * 0.004f;
    int numDrops = static_cast<int>(rainPercent * srcDescPtr->h * srcDescPtr->w);
    ...
    std::cout<<"\n numDrops "<<numDrops << " "<< srcDescPtr->h << " " << srcDescPtr->w;

    for (int i = 0; i < numDrops; i++)
    {
        int xStart = rand() % (srcDescPtr->w - std::abs(slant));
        int yStart = rand() % (srcDescPtr->h - rainHeight);
        for (int j = 0; j < rainHeight; j++)// height -  drop length
        {
            ...
            if (x >= 0 && x < srcDescPtr->w && y < srcDescPtr->h)
            {
                ...
                {
                    int idx = k * layoutParams.bufferMultiplier;
    ...
    for (int batchCount = 0; batchCount < dstDescPtr->n; batchCount++)
    {
        ...
        if ((srcDescPtr->c == 3) && (srcDescPtr->layout == RpptLayout::NHWC) && (dstDescPtr->layout == RpptLayout::NCHW))
        {
            ...
            for(int i = 0; i < roi.xywhROI.roiHeight; i++)
            {
                ...
                for (; vectorLoopCount < alignedLength; vectorLoopCount += 48)
                {
                    ...
                    p1[0] = _mm256_add_ps(_mm256_mul_ps(_mm256_sub_ps(avx_p1, pMul), p1[0]), _mm256_mul_ps(pMul, p2[0]));    // alpha-blending adjustment
                    p1[1] = _mm256_add_ps(_mm256_mul_ps(_mm256_sub_ps(avx_p1, pMul), p1[1]), _mm256_mul_ps(pMul, p2[1]));    // alpha-blending adjustment
                    p1[2] = _mm256_add_ps(_mm256_mul_ps(_mm256_sub_ps(avx_p1, pMul), p1[2]), _mm256_mul_ps(pMul, p2[2]));    // alpha-blending adjustment
                    p1[3] = _mm256_add_ps(_mm256_mul_ps(_mm256_sub_ps(avx_p1, pMul), p1[3]), _mm256_mul_ps(pMul, p2[3]));    // alpha-blending adjustment
                    p1[4] = _mm256_add_ps(_mm256_mul_ps(_mm256_sub_ps(avx_p1, pMul), p1[4]), _mm256_mul_ps(pMul, p2[4]));    // alpha-blending adjustment
                    p1[5] = _mm256_add_ps(_mm256_mul_ps(_mm256_sub_ps(avx_p1, pMul), p1[5]), _mm256_mul_ps(pMul, p2[5]));    // alpha-blending adjustment
                ...
                {
                    *dstPtrTempR = static_cast<Rpp8u>((srcPtr1Temp[0] * (1 - alpha) + srcPtr2Temp[0] * alpha));
                    *dstPtrTempG = static_cast<Rpp8u>((srcPtr1Temp[1] * (1 - alpha) + srcPtr2Temp[1] * alpha));
                    *dstPtrTempB = static_cast<Rpp8u>((srcPtr1Temp[2] * (1 - alpha) + srcPtr2Temp[2] * alpha));;
        ...
        else if ((srcDescPtr->c == 3) && (srcDescPtr->layout == RpptLayout::NCHW) && (dstDescPtr->layout == RpptLayout::NHWC))
        {
            ...
            for(int i = 0; i < roi.xywhROI.roiHeight; i++)
            {
                ...
                {
                    dstPtrTemp[0] = static_cast<Rpp8u>((*srcPtr1TempR * (1 - alpha) + *srcPtr2TempR * alpha));
                    dstPtrTemp[1] = static_cast<Rpp8u>((*srcPtr1TempG * (1 - alpha) + *srcPtr2TempG * alpha));
                    dstPtrTemp[2] = static_cast<Rpp8u>((*srcPtr1TempB * (1 - alpha) + *srcPtr2TempB * alpha));
        ...
        else
        {
            ...
            for(int c = 0; c < layoutParams.channelParam; c++)
            {
                ...
                for(int i = 0; i < roi.xywhROI.roiHeight; i++)
                {
                    ...
                    for (; vectorLoopCount < bufferLength; vectorLoopCount++)
                    {
                        *dstPtrTemp = static_cast<Rpp8u>((*srcPtr1Temp * (1 - alpha) + *srcPtr2Temp * alpha));
