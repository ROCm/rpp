...
// Gridmask helpers - Returning mask pixel values
...
// PKD3 -> PLN3
template <typename T>
__device__ void gridmask_result_pkd3_pln3_hip_compute(T *srcPtr, int srcIdx, T *dstPtr, int dstIdx, uint3 *dstStridesNCH, float2 *gridColRatio, float gridRatio)
{
    if ((gridColRatio->x - floorf(gridColRatio->x) >= gridRatio) || (gridColRatio->y - floorf(gridColRatio->y) >= gridRatio))
    {
        dstPtr[dstIdx] = srcPtr[srcIdx];
...
// PLN3 -> PKD3
template <typename T>
__device__ void gridmask_result_pln3_pkd3_hip_compute(T *srcPtr, int srcIdx, uint3 *srcStridesNCH, T *dstPtr, int dstIdx, float2 *gridColRatio, float gridRatio)
{
    if ((gridColRatio->x - floorf(gridColRatio->x) >= gridRatio) || (gridColRatio->y - floorf(gridColRatio->y) >= gridRatio))
    {
        dstPtr[dstIdx] = srcPtr[srcIdx];
                                          ...
                                          RpptROIPtr roiTensorPtrSrc)
{
    ...
    float2 gridRowRatio, gridColRatio;
    gridmask_ratio_hip_compute(id_x, id_y, &rotateRatios, &translateRatios, &gridRowRatio, &gridColRatio);
    gridmask_result_pkd3_pln3_hip_compute(srcPtr, srcIdx, dstPtr, dstIdx, &dstStridesNCH, &gridColRatio, gridRatio);
}
                                          ...
                                          RpptROIPtr roiTensorPtrSrc)
{
    ...
    float2 gridRowRatio, gridColRatio;
    gridmask_ratio_hip_compute(id_x, id_y, &rotateRatios, &translateRatios, &gridRowRatio, &gridColRatio);
    gridmask_result_pln3_pkd3_hip_compute(srcPtr, srcIdx, &srcStridesNCH, dstPtr, dstIdx, &gridColRatio, gridRatio);
}
