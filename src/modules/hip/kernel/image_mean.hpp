...
__global__ void image_mean_grid_result_tensor(float *srcPtr,
                                              uint xBufferLength,
                                              float *dstPtr,
                                              RpptROIPtr roiTensorPtrSrc)
{
    ...
    if (hipThreadIdx_x == 0)
    {
        ...
        dstPtr[hipBlockIdx_z] = partialSumLDS[0] / totalElements;
    }
}

__global__ void image_mean_grid_3channel_result_tensor(float *srcPtr,
                                                       uint xBufferLength,
                                                       float *dstPtr,
                                                       RpptROIPtr roiTensorPtrSrc)
{
    ...
    if (hipThreadIdx_x == 0)
    {
        ...
        dstPtr[idx] = partialRSumLDS[0] / totalElements;
        dstPtr[idx + 1] = partialGSumLDS[0] / totalElements;
        dstPtr[idx + 2] = partialBSumLDS[0] / totalElements;
...
template <typename T, typename U>
RppStatus hip_exec_image_mean_tensor(T *srcPtr,
                                     ...
                                     rpp::Handle& handle)
{
    ...
    if ((srcDescPtr->c == 1) && (srcDescPtr->layout == RpptLayout::NCHW))
    {
        ...
        hipLaunchKernelGGL(image_sum_pln1_tensor,
                           ...
                           roiTensorPtrSrc);
        hipStreamSynchronize(handle.GetStream());
        hipLaunchKernelGGL(image_mean_grid_result_tensor,
                           dim3(1, 1, gridDim_z),
                           dim3(1024, 1, 1),
                           0,
                           handle.GetStream(),
    ...
    else if ((srcDescPtr->c == 3) && (srcDescPtr->layout == RpptLayout::NCHW))
    {
        ...
        hipLaunchKernelGGL(image_sum_pln3_tensor,
                           ...
                           roiTensorPtrSrc);
        hipStreamSynchronize(handle.GetStream());
        hipLaunchKernelGGL(image_mean_grid_3channel_result_tensor,
                           dim3(1, 1, gridDim_z),
                           dim3(1024, 1, 1),
                           0,
                           handle.GetStream(),
