...
* Changed API of swap_channels augmentation to be called channel_permute, which now accepts one new argument, "permutationsList" (pointer to a uint array) that provides the permutation order to swap the RGB channels of each input image in the batch in any order. (#547)
  ...
  * New API - `RppStatus rppt_channel_permute_host(RppPtr_t srcPtr, RpptDescPtr srcDescPtr, RppPtr_t dstPtr, RpptDescPtr dstDescPtr, Rpp32u *permutationsList , rppHandle_t rppHandle);`
