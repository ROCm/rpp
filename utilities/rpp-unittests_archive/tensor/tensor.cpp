...
int main(int argc, char **argv)
{
    ...
    {
    case 1:
        strcpy(funcName, "tensor_add");
        break;
    case 2:
        strcpy(funcName, "tensor_sub");
        break;
    case 3:
    ...
    Rpp8u *tensor_input = (Rpp8u *)calloc(TENSOR_SIZE, sizeof(Rpp8u));
    Rpp8u *tensor_input2 = (Rpp8u *)calloc(TENSOR_SIZE, sizeof(Rpp8u));
    Rpp8u *tensor_output = (Rpp8u *)calloc(TENSOR_SIZE, sizeof(Rpp8u));
    Rpp8u *tensor_compare = (Rpp8u *)calloc(TENSOR_SIZE, sizeof(Rpp8u));
    ...
    err |= clEnqueueWriteBuffer(theQueue, d_tensor_input, CL_TRUE, 0, TENSOR_SIZE * sizeof(Rpp8u), tensor_input, 0, NULL, NULL);
    ...
    case 1:
    {
        ...
        start = clock();
        if (ip_bitDepth == 0)
            rppi_tensor_add_u8_gpu(d_tensor_input, d_tensor_input2, d_tensor_output, 4, tensor_dims, handle);
        end = clock();
        for(int i = 0; i < TENSOR_SIZE; i++)
    ...
    case 2:
    {
        ...
        start = clock();
        if (ip_bitDepth == 0)
            rppi_tensor_subtract_u8_gpu(d_tensor_input, d_tensor_input2, d_tensor_output, 4, tensor_dims, handle);
        end = clock();
        for(int i = 0; i < TENSOR_SIZE; i++)
            tensor_compare[i] = tensor_input[i] - tensor_input2[i];
        clEnqueueReadBuffer(theQueue, d_tensor_output, CL_TRUE, 0, TENSOR_SIZE * sizeof(Rpp8u), tensor_output, 0, NULL, NULL);
