...
int main(int argc, char **argv)
{
    ...
    case 17:
    {
        ...
        start_omp = omp_get_wtime();
        if (ip_bitDepth == 0)
            rppi_rotate_u8_pkd3_batchPD_host(input, srcSize, maxSize, output, dstSize, maxDstSize, angle, 0, noOfImages, handle);
        else if (ip_bitDepth == 1)
            rppi_rotate_f16_pkd3_batchPD_host(inputf16, srcSize, maxSize, outputf16, dstSize, maxDstSize, angle, 0, noOfImages, handle);
        else if (ip_bitDepth == 2)
            rppi_rotate_f32_pkd3_batchPD_host(inputf32, srcSize, maxSize, outputf32, dstSize, maxDstSize, angle, 0, noOfImages, handle);
        end_omp = omp_get_wtime();
        end = clock();
    ...
    case 18:
    {
        ...
        start_omp = omp_get_wtime();
        if (ip_bitDepth == 0)
            rppi_warp_affine_u8_pkd3_batchPD_host(input, srcSize, maxSize, output, dstSize, maxDstSize, affine_array, noOfImages, handle);
        else if (ip_bitDepth == 1)
            missingFuncFlag = 1;
        else if (ip_bitDepth == 2)
            missingFuncFlag = 1;
    ...
    case 19:
    {
        ...
        start_omp = omp_get_wtime();
        if (ip_bitDepth == 0)
            rppi_resize_u8_pkd3_batchPD_host(input, srcSize, maxSize, output, dstSize, maxDstSize, 0, noOfImages, handle);
        else if (ip_bitDepth == 1)
            rppi_resize_f16_pkd3_batchPD_host(inputf16, srcSize, maxSize, outputf16, dstSize, maxDstSize, 0, noOfImages, handle);
        else if (ip_bitDepth == 2)
            rppi_resize_f32_pkd3_batchPD_host(inputf32, srcSize, maxSize, outputf32, dstSize, maxDstSize, 0, noOfImages, handle);
        end_omp = omp_get_wtime();
        end = clock();
    ...
    case 20:
    {
        ...
        start_omp = omp_get_wtime();
        if (ip_bitDepth == 0)
            rppi_resize_crop_u8_pkd3_batchPD_host(input, srcSize, maxSize, output, dstSize, maxDstSize, x1, x2, y1, y2, 0, noOfImages, handle);
        else if (ip_bitDepth == 1)
            rppi_resize_crop_f16_pkd3_batchPD_host(inputf16, srcSize, maxSize, outputf16, dstSize, maxDstSize, x1, x2, y1, y2, 0, noOfImages, handle);
        else if (ip_bitDepth == 2)
            rppi_resize_crop_f32_pkd3_batchPD_host(inputf32, srcSize, maxSize, outputf32, dstSize, maxDstSize, x1, x2, y1, y2, 0, noOfImages, handle);
        end_omp = omp_get_wtime();
        end = clock();
    ...
    case 34:
    {
        ...
        start_omp = omp_get_wtime();
        if (ip_bitDepth == 0)
            rppi_inclusive_OR_u8_pkd3_batchPD_gpu(input, input_second, srcSize, maxSize, output, noOfImages, handle);
        else if (ip_bitDepth == 1)
            missingFuncFlag = 1;
        else if (ip_bitDepth == 2)
            missingFuncFlag = 1;
    ...
    case 38:
    {
        ...
        start_omp = omp_get_wtime();
        if (ip_bitDepth == 0)
            rppi_channel_extract_u8_pkd3_batchPD_host(input, srcSize, maxSize, output, extractChannelNumber, noOfImages, handle);
        else if (ip_bitDepth == 1)
            missingFuncFlag = 1;
        else if (ip_bitDepth == 2)
            missingFuncFlag = 1;
    ...
    case 61:
    {
        ...
        start_omp = omp_get_wtime();
        if (ip_bitDepth == 0)
            rppi_resize_crop_mirror_u8_pkd3_batchPD_host(input, srcSize, maxSize, output, dstSize, maxDstSize, x1, x2, y1, y2, mirrorFlag, 0, noOfImages, handle);
        else if (ip_bitDepth == 1)
            rppi_resize_crop_mirror_f16_pkd3_batchPD_host(inputf16, srcSize, maxSize, outputf16, dstSize, maxDstSize, x1, x2, y1, y2, mirrorFlag, 0, noOfImages, handle);
        else if (ip_bitDepth == 2)
            rppi_resize_crop_mirror_f32_pkd3_batchPD_host(inputf32, srcSize, maxSize, outputf32, dstSize, maxDstSize, x1, x2, y1, y2, mirrorFlag, 0, noOfImages, handle);
        end_omp = omp_get_wtime();
        end = clock();
    ...
    case 62:
    {
        ...
        start_omp = omp_get_wtime();
        if (ip_bitDepth == 0)
            rppi_crop_u8_pkd3_batchPD_host(input, srcSize, maxSize, output, dstSize, maxDstSize, crop_pos_x, crop_pos_y, 0, noOfImages, handle);
        else if (ip_bitDepth == 1)
            rppi_crop_f16_pkd3_batchPD_host(inputf16, srcSize, maxSize, outputf16, dstSize, maxDstSize, crop_pos_x, crop_pos_y, 0, noOfImages, handle);
        else if (ip_bitDepth == 2)
            rppi_crop_f32_pkd3_batchPD_host(inputf32, srcSize, maxSize, outputf32, dstSize, maxDstSize, crop_pos_x, crop_pos_y, 0, noOfImages, handle);
        end_omp = omp_get_wtime();
        end = clock();
    ...
    case 64:
    {
        ...
        start_omp = omp_get_wtime();
        if (ip_bitDepth == 0)
            rppi_color_twist_u8_pkd3_batchPD_host(input, srcSize, maxSize, output, alpha, beta, hueShift, saturationFactor, 0, noOfImages, handle);
        else if (ip_bitDepth == 1)
            rppi_color_twist_f16_pkd3_batchPD_host(inputf16, srcSize, maxSize, outputf16, alpha, beta, hueShift, saturationFactor, 0, noOfImages, handle);
        else if (ip_bitDepth == 2)
            rppi_color_twist_f32_pkd3_batchPD_host(inputf32, srcSize, maxSize, outputf32, alpha, beta, hueShift, saturationFactor, 0, noOfImages, handle);
        end_omp = omp_get_wtime();
        end = clock();
