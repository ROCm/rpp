...
int main(int argc, char **argv)
{
    ...
    case 73:
    {
        ...
        start_omp = omp_get_wtime();
        if (ip_bitDepth == 0)
            rppi_hog_u8_pln3_batchPD_host(input, srcSize, maxSize, binsTensor, binsTensorLength, kernelSize, windowSize, windowStride, numOfBins, noOfImages, handle);
        else if (ip_bitDepth == 1)
            missingFuncFlag = 1;
        else if (ip_bitDepth == 2)
            missingFuncFlag = 1;
        ...
        printf("\nPrinting the bins tensor for hog_pln3:");
