...
char funcType[1000] = {"BatchPD_ROID_Center"};
...
int main(int argc, char **argv)
{
    ...
    closedir(dr2); 
	const int size_affine = images * 6;
    Rpp32f affine[size_affine];
    for(i = 0 ; i < images ; i++)
	{
        affine[0 + i * 6] = 1;
        affine[1 + i * 6] = 0.5;
	...
	rppi_warp_affine_u8_pkd3_batchPD_ROID_gpu(d_input, srcSize,maxSize, d_output, dstSize,maxDstSize, affine,roiPoints,noOfImages, handle);

    end = clock();
	cpu_time_used = ((double) (end - start)) / CLOCKS_PER_SEC;
	cout<<" BatchPD_ROID_Center : "<<cpu_time_used<<endl;  
