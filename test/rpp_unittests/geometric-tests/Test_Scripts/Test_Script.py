...
def process_csv(csv_reader,numline):
    ...
    for count,row in enumerate(csv_reader):
        ...
        for file_name in sc.files:
            ...
            if ("BatchPS" in file_name):
                code = code + sc.ps_ioBuffer
            elif ("BatchP" in file_name):
                code = code + sc.padding_ioBuffer
            elif("BatchDS" in file_name):
                code = code + sc.non_padding_ioBuffer_for_DS
            else:
            ...
            if ("BatchPD" in file_name):
                code = code + "dstSize, maxDstSize, "
            elif ("BatchDD" in file_name or "BatchSD" in file_name):
                code = code + "dstSize, "
            elif ("BatchPS" in file_name):
                code = code + "dstSize[0], dstSize[0],"
            else:
                code = code + "dstSize[0], "
